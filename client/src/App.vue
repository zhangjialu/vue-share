      ...
      <v-card dark v-if="searchResults.length" id="search__card">
        <v-list>
          <v-list-tile v-for="result in searchResults" :key="result._id" @click="goToSearchResult(result._id)">
            <v-list-tile-title>
              {{result.title}}
              <span class="font-weight-thin">{{result.description | clipping(20) }}</span>
            </v-list-tile-title>
    ...
    <main>
      <v-container class="mt-4">
        ...
        <v-snackbar v-model="authSnackbar" color="success" :timeout="5000" bottom left>
          ...
          <header>You are now signed in!</header>
          <v-btn flat dark @click="authSnackbar = false">Close</v-btn>
        </v-snackbar>
        <!-- Auth Error Snackbar -->
        <v-snackbar v-if="authError" v-model="authErrorSnackbar" color="error" :timeout="5000" bottom left>
          <v-icon class="mr-3">cancel</v-icon>
          <header>{{authError.message}}</header>
          <v-btn flat dark to="/signin">Signin</v-btn>
        </v-snackbar>
      </v-container>
    </main>
  </v-app>
  ...
  computed: {
    ...
    sideNavItems() {
      ...
      if (this.user) {
        items = [
          { icon: 'chat', title: 'Posts', link: '/posts' },
          { icon: 'stars', title: 'Add Post', link: '/post/add' },
          { icon: 'account_box', title: 'Profile', link: '/profile' }
        ]
      }
      return items
