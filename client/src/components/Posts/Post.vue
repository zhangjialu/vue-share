      ...
      <v-flex xs12>
        <v-card hover>
          <v-card-title>
            ...
            <v-spacer></v-spacer>
            <v-icon @click="goToPreviousPage" color="primary" large>arrow_back</v-icon>
          </v-card-title>
          ...
          <!-- Post Image Dialog (Enlarge Post Image) -->
      ...
      <v-layout class="mb-3" v-if="user">
        <v-flex xs12>
          <v-form v-model="isFormValid" lazy-validation ref="form" @submit.prevent="handleAddPostMessage">
            <v-layout row>
              <v-flex xs12>
                <v-text-field :rules="messageRules" v-model="messageBody" clearable :append-outer-icon="messageBody && 'send'" @click:append-outer="handleAddPostMessage" label="Add Message" type="text" prepend-icon="email" required></v-text-field>
              </v-flex>
            </v-layout>
          </v-form>
        </v-flex>
              ...
              <v-list-tile avatar inset :key="message.title">
                ...
                <v-list-tile-content>
                  ...
                  <v-list-tile-sub-title>
                    ...
                    <span class="grey--text text-lighten-1 hidden-xs-only">{{getTimeFormNow(message.messageDate)}}</span>
                  </v-list-tile-sub-title>
                </v-list-tile-content>
...
export default {
  ...
  methods: {
    ...
    getTimeFormNow(timeStr) {
      return moment(new Date(timeStr)).fromNow()
    },
    checkIfPostLiked(postId) {
      // check if uesr favorites includes post with id of 'postId'
      if (
        this.userFavorites &&
